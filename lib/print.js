...
export default async function (m, conn = { user: {} }) {
    ...
    console.log(`╭⋄─────────────────────────────────⋄
│𝙉𝙖𝙪𝙛𝙧𝙖𝙕𝙖𝙥𝙥-𝙈𝘿 : ${chalk.white(chalk.bgGreenBright('%s'))}
╰⋄─────────────────────────────────⋄
╭⋄─⧐ ${chalk.white(chalk.bgBlackBright('TIEMPO:'))}
╰─⋄─⧐${chalk.cyanBright(oraItaliana)}

╭⋄─⧐ ${chalk.white(chalk.bgBlueBright('USUARIO:'))}
╰─⋄─⧐ ${chalk.greenBright('%s')}

╭⋄─⧐ ${chalk.white(chalk.bgOrangeBright('TIPO DE CHAT:'))}
╰─⋄─⧐ ${chalk.cyanBright(chatName)}

${chalk.yellowBright('Type:')} ${chalk.cyanBright('%s')} 💬
`.trim(),
