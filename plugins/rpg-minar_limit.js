...
global.db.data.users[m.sender].exp += d * 1  
let time = global.db.data.users[m.sender].lastdiamantes + 600000
if (new Date - global.db.data.users[m.sender].lastdiamantes < 600000) throw `*『⛏️』Por favor, espere* ${msToTime(time - new Date())} *para volver a minar diamantes. ⛏️*`  

global.db.data.users[m.sender].lastdiamantes = new Date * 1  
  

conn.reply(m.chat, `${minar} \`${d}\` *de NaufraCoins* 🪙`, fkontak, m)
}
handler.help = ['minar']
handler.tags = ['rol']
