...
let text
    if (args.length >= 1) {
        ...
        text = m.quoted.text;
    } else throw "*『🪐』Debe de ingresar cualquier texto.*";
   if (!text) return m.reply('*『🪐』Debe de ingresar cualquier texto.*');
    const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
    const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
    const mishi = text.replace(mentionRegex, '');
   if (mishi.length > 30) return m.reply('*『❌』El texto contiene muchos caracteres, maximo 30.*');
