...
export async function before(m, {isAdmin, isBotAdmin, isOwner}) {
  ...
  if (user.warn >= 5) {
    user.warn = 0;
    await m.reply(`『✦』 *Hola @${m.sender.split`@`[0]}, superaste las 5 advertencias por lo que serás eliminado de este grupo por tu mala conducta.*`, false, {mentions: [m.sender]});
    user.banned = true;
