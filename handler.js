...
m.exp += xp
if (!isPrems && plugin.limit && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
...
if (m.limit)
m.reply(`『💎』Se gasto *${+m.limit}* diamante(s).`)
}
break
}
}
...
await this.sendMessage(cs.from, { contacts: { displayName: '𝘕𝘢𝘶𝘧𝘳𝘢𝘡𝘢𝘱𝘱', contacts: [{ vcard }] }}, {quoted: callmsg})
...
if (msg) return conn.reply(m.chat, msg, m, { contextInfo:{ externalAdReply: {title: wm, body: '❗ 𝗔𝗗𝗩𝗘𝗥𝗧𝗘𝗡𝗖𝗜𝗔 ❗', sourceUrl: global.canal, thumbnailUrl: naufraAll.getRandom()}}})
